...
export function MatchHistory() {
    ...
    const fetchMatchDetails = async (matchId: string): Promise<PlayerStats[]> => {
        ...
        return [
            { name: 'TenZ', agent: 'Jett', kills: 24, deaths: 15, assists: 8, acs: 298, headshot: 32 },
            { name: 'SicK', agent: 'Sage', kills: 18, deaths: 14, assists: 12, acs: 245, headshot: 28 },
            { name: 'dapr', agent: 'Cypher', kills: 16, deaths: 16, assists: 9, acs: 221, headshot: 24 },
            { name: 'ShahZaM', agent: 'Sova', kills: 15, deaths: 17, assists: 14, acs: 210, headshot: 26 },
            { name: 'zombs', agent: 'Omen', kills: 12, deaths: 18, assists: 10, acs: 189, headshot: 22 },
            { name: 'Enemy1', agent: 'Reyna', kills: 20, deaths: 19, assists: 5, acs: 267, headshot: 30 },
            { name: 'Enemy2', agent: 'Killjoy', kills: 14, deaths: 17, assists: 8, acs: 198, headshot: 25 },
            { name: 'Enemy3', agent: 'Breach', kills: 13, deaths: 18, assists: 11, acs: 187, headshot: 21 },
            { name: 'Enemy4', agent: 'Phoenix', kills: 16, deaths: 16, assists: 7, acs: 203, headshot: 27 },
            { name: 'Enemy5', agent: 'Brimstone', kills: 11, deaths: 15, assists: 9, acs: 175, headshot: 19 },
        ];
    };
    ...
    const handleMatchClick = async (matchId: string) => {
        ...
        try {
            ...
            setMatches((prev) =>
                prev.map((m) => (m.id === matchId ? { ...m, players: playerStats } : m))
            );
            setExpandedMatch(matchId);
        } catch (error) {
        ...
        <div className="bg-[#0f0f0f] border border-[#1a1a1a] rounded-lg overflow-hidden">
            ...
            <div className="p-6 space-y-4">
                {isInitialLoading ? (
                    ...
                    <>
                        {matches.map((match) => {
                            const matchBgStyle: React.CSSProperties | undefined = match.mapId
                                ? {
                            ...
                            return (
                                ...
                                >
                                    <div>
                                        <CollapsibleTrigger
                                            className={`w-full text-left p-0 transition-colors relative overflow-hidden ${
                                                ...
                                                match.result === 'Victory' ? 'border-[#4ade80]' : 'border-[#f87171]'
                                            }`}
                                            style={{ padding: 0 }}
                                        ...
                                        >
                                            <div className="relative p-5" style={matchBgStyle}>
                                                ...
                                                <div
                                                    ...
                                                    style={{
                                                        backgroundColor:
                                                            match.result === 'Victory' ? 'rgba(74, 222, 128, 0.1)' : 'rgba(248, 113, 113, 0.1)',
                                                    }}
                                                />
                                                {/* Hover overlay */}
                                                <div
                                                    ...
                                                    style={{
                                                        backgroundColor:
                                                            match.result === 'Victory' ? 'rgba(74, 222, 128, 0.2)' : 'rgba(248, 113, 113, 0.2)',
                                                ...
                                                <div className="flex items-center justify-between relative z-30">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-12 h-12 bg-gradient-to-br from-[#4a7cff] to-[#2d5acc] rounded-lg flex items-center justify-center flex-shrink-0">
                                                            <span className="text-sm">{match.agent[0]}</span>
                                                        </div>
                                  ...
                                  {match.result}
                                                                ...
                                                                <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-black/25">
                                                                    <img
                                                                        src="https://media.valorant-api.com/competitivetiers/03621f52-342b-cf4e-4f86-9350a49c6d04/27/smallicon.png"
                                                                        alt="Rank Icon"
                                                                        className="w-3 h-3"
                                                                    />
                                                                    <span className="text-xs text-gray-300">{match.rank}</span>
                                                                </div>
                                                                ...
                                                                >
                                                                    {match.rrChange > 0 ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                                                                    <span className="text-xs">{match.rrChange > 0 ? '+' : ''}{match.rrChange} RR</span>
                                                                </div>
                                                            </div>
                                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div
                                                        className={`p-5 border-l border-r ${
                                                            match.result === 'Victory' ? 'bg-[#4ade80]/10 border-[#4ade80]' : 'bg-[#f87171]/10 border-[#f87171]'
                                                        }`}
                                                    >
                                                        <h4 className="text-sm text-gray-400 mb-3">
                                                            {match.result === 'Victory' ? 'Your Team (Victory)' : 'Enemy Team (Victory)'}
                                                        </h4>
                                                        <div className="space-y-2">
                                                            {match.players.slice(0, 5).map((player, idx) => (
                                                                <div key={idx} className="flex items-center justify-between p-3 bg-black/20 rounded-lg">
                                                                    <div className="flex items-center gap-4">
                                                                        <div className="w-8 h-8 bg-gradient-to-br from-[#4a7cff] to-[#2d5acc] rounded flex items-center justify-center">
                                                                    ...
                                                                    <div className="flex items-center gap-6 text-sm">
                                                                        <div className="text-center">
                                                                            ...
                                                                            <div className="text-white">{player.kills}/{player.deaths}/{player.assists}</div>
                                                                        </div>
                                                                        <div className="text-center">
                                                                            <div className="text-gray-400 text-xs">ACS</div>
                                                                            <div className="text-white">{player.acs}</div>
                                                                        </div>
                                                                        <div className="text-center">
                                                                            <div className="text-gray-400 text-xs">HS%</div>
                                                                            <div className="text-white">{player.headshot}%</div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            ))}
                                                    ...
                                                    <div
                                                        className={`p-5 border-l border-r border-b rounded-b-lg ${
                                                            match.result === 'Victory' ? 'bg-[#f87171]/10 border-[#f87171]' : 'bg-[#4ade80]/10 border-[#4ade80]'
                                                        }`}
                                                    >
                                                        <h4 className="text-sm text-gray-400 mb-3">
                                                            {match.result === 'Victory' ? 'Enemy Team (Defeat)' : 'Your Team (Defeat)'}
                                                        </h4>
                                                        <div className="space-y-2">
                                                            {match.players.slice(5, 10).map((player, idx) => (
                                                                <div key={idx} className="flex items-center justify-between p-3 bg-black/20 rounded-lg">
                                                                    <div className="flex items-center gap-4">
                                                                        <div className="w-8 h-8 bg-gradient-to-br from-[#f87171] to-[#dc2626] rounded flex items-center justify-center">
                                                                            <span className="text-xs">{player.agent[0]}</span>
                                                                        </div>
                                                                        <div>
                                                                            <div className="text-white text-sm">{player.name}</div>
